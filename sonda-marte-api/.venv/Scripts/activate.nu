...
export-env {
    ...
    let virtual_env = r#'C:\Users\Usuário\Documents\sonda-marte-api\sonda-marte-api\sonda-marte-api\.venv'#
    let bin = r#'Scripts'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'sonda-marte-api-py3.14'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'sonda-marte-api-py3.14'#
    }
